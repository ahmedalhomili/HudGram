...
public class HudLanguagesSelectActivity extends BaseHudSettingsActivity {
    ...
    private final ArrayList<LocaleInfo> allLanguages = new ArrayList<>();
    private final ArrayList<LocaleInfo> sortedLanguages = new ArrayList<>();
    private final ArrayList<LocaleInfo> searchResult = new ArrayList<>();
    ...
    public HudLanguagesSelectActivity(int type) {
    ...
    private void search(final String text) {
        ...
        Utilities.searchQueue.postRunnable(searchRunnable = new Runnable() {
            @Override
            public void run() {
                final ArrayList<LocaleInfo> results = new ArrayList<>();
                String lowerQuery = text.toLowerCase();
                for (LocaleInfo language : allLanguages) {
        ...
        Collections.sort(sortedLanguages, (o1, o2) -> {
            if (currentType == TYPE_TARGET) {
                if (o1.langCode.equals("app")) {
                    ...
                    return 1;
                } else if (HudConfig.translationTarget.equals(o1.langCode)) {
                    return -1;
                } else if (HudConfig.translationTarget.equals(o2.langCode)) {
                    return 1;
                }
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        Class<? extends UItem.UItemFactory> factory = currentType == TYPE_TARGET ? TextRadioCellFactory.class : TextCheckbox2CellFactory.class;
        for (LocaleInfo locale : (searchWas ? searchResult : sortedLanguages)) {
            if (locale.langCode.equals("shadow")) {
                items.add(UItem.asShadow(null));
                continue;
            }
            UItem item = UItem.ofFactory(factory);
            item.id = sortedLanguages.indexOf(locale);
            if (locale.langCode.equals("app")) {
                item.text = getString("TranslationTargetApp");
            } else {
                item.text = locale.name;
                item.subtext = locale.nameLocalized;
            }
            item.checked = currentType == TYPE_RESTRICTED ? restrictedLanguages.contains(locale.langCode) : HudConfig.translationTarget.equals(locale.langCode);
            items.add(item);
        }
        items.add(UItem.asShadow(null));
    }
    ...
    protected void onItemClick(UItem item, View view, int position, float x, float y) {
        ...
        if (localeInfo != null) {
            if (currentType == TYPE_RESTRICTED) {
                ...
                if (remove) {
                    restrictedLanguages.remove(localeInfo.langCode);
                } else {
                ...
                HudConfig.setTranslationTarget(localeInfo.langCode);
                finishFragment();
            }
        }
    }
    ...
    protected String getActionBarTitle() {
        return currentType == TYPE_RESTRICTED ? getString("DoNotTranslate") : getString("TranslationTarget");
    }
    ...
    protected static class TextRadioCellFactory extends UItem.UItemFactory<TextRadioCell> {
        ...
        public TextRadioCell createView(Context context, RecyclerListView listView, int currentAccount, int classGuid, Theme.ResourcesProvider resourcesProvider) {
            return new TextRadioCell(context);
        }

        @Override
        public void bindView(View view, UItem item, boolean divider, UniversalAdapter adapter, UniversalRecyclerView listView) {
            TextRadioCell cell = (TextRadioCell) view;
            if (TextUtils.isEmpty(item.subtext)) {
                cell.setTextAndCheck(item.text == null ? null : item.text.toString(), item.checked, divider);
            } else {
                cell.setTextAndValueAndCheck(item.text == null ? null : item.text.toString(), item.subtext == null ? null : item.subtext.toString(), item.checked, false, divider);
            }
        }
    }
