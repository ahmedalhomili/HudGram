...
/**
 * Manages multiple reply texts for a single DM rule.
 * Unlike HudAutoReplyMessagesActivity which saves to HudConfig globally,
 * this uses a callback to return the edited list to HudAutoReplyDMAddActivity.
 */
public class HudAutoReplyDMMessagesActivity extends BaseHudSettingsActivity {

    public interface OnMessagesChangedListener {
        void onMessagesChanged(ArrayList<String> messages);
    }

    private ArrayList<String> messages;
    private final OnMessagesChangedListener listener;

    public HudAutoReplyDMMessagesActivity(ArrayList<String> existingMessages, OnMessagesChangedListener listener) {
        this.messages = new ArrayList<>(existingMessages != null ? existingMessages : new ArrayList<>());
        this.listener = listener;
    }

    @Override
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        items.add(UItem.asShadow(getString("HudAutoReplyMultipleHeader")));
        ...
        if (messages.isEmpty()) {
            ...
            for (int i = 0; i < messages.size(); i++) {
                ...
                UItem uItem = DMMessageCellFactory.of(100 + i, (i + 1) + ".", messages.get(i));
    ...
    private void notifyParent() {
        if (listener != null) {
            listener.onMessagesChanged(new ArrayList<>(messages));
        }
    }
    ...
    // === DM Message cell factory ===
    protected static class DMMessageCellFactory extends UItem.UItemFactory<DMMessageCell> {
        static {
            setup(new DMMessageCellFactory());
        }

        @Override
        public DMMessageCell createView(Context context, RecyclerListView listView, int currentAccount, int classGuid, Theme.ResourcesProvider resourcesProvider) {
            return new DMMessageCell(context);
        }

        @Override
        public void bindView(View view, UItem item, boolean divider, UniversalAdapter adapter, UniversalRecyclerView listView) {
            DMMessageCell cell = (DMMessageCell) view;
        ...
        public static UItem of(int id, CharSequence number, CharSequence message) {
            UItem item = UItem.ofFactory(DMMessageCellFactory.class);
            item.id = id;
            item.text = number;
    ...
    protected static class DMMessageCell extends FrameLayout {
        private final TextView numberView;
        private final TextView messageView;
        private boolean needsDivider;

        public DMMessageCell(Context context) {
            super(context);
